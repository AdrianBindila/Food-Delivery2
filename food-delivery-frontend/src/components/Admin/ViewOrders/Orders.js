...
function Orders() {
  ...
  const initialOrders = JSON.parse(sessionStorage.getItem("restaurantOrders"));
  ...
  function filterOrders(filter) {
    const newOrders = initialOrders.filter((order) => {
  ...
  return (
    <>
      <OrderFilter filter={filterOrders} status={status} />
      <Table>
        <thead>
          <tr>
            <th>Customer</th>
            ...
            orders.map((order, index) => {
              return (
                <tr key={index}>
                  ...
                  <td>
                    {order.date[0] + ".0" + order.date[1] + order.date[2]}
                  </td>
                  ...
                  <td>
                    {order.status === status.pending ? (
                      <>
                        ...
                        <Button
                          onClick={() =>
                            changeStatus(order, index, status.declined)
                          }
                        >
